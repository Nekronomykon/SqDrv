...
//------------------------------------------------------------------------------
vtkImageData *AcquireFileCUBE::GetGridOutput()
{
...
//------------------------------------------------------------------------------
int AcquireFileCUBE::RequestInformation(vtkInformation *vtkNotUsed(request),
                                        ...
                                        vtkInformationVector *vtkNotUsed(outputVector))
{
    ...
    vtksys::ifstream file_in(this->GetFileName());

    if (!file_in.is_open())
    {
        vtkErrorMacro("AcquireFileCUBE error opening file: " << this->GetFileName());
        return 0;
    }

    getline(file_in, title); // first title line
    getline(file_in, title); // second title line with name of scalar field?
    ...
    {
        vtkErrorMacro("AcquireFileCUBE error reading file: " << this->GetFileName()
...
//------------------------------------------------------------------------------
int AcquireFileCUBE::RequestData(vtkInformation *vtkNotUsed(request),
                                 vtkInformationVector **vtkNotUsed(inputVector),
                                 vtkInformationVector *outputVector)
{
    ...
    // Read in number of atoms, x-origin, y-origin z-origin
    //
    if (!(file_in >> nAtoms >> elements[3] >> elements[7] >> elements[11]))
    {
        vtkErrorMacro("AcquireFileCUBE error reading file: "
                      << this->GetFileName() << " Premature EOF while reading atoms, x-origin y-origin z-origin.");
        file_in.close();
        return 0;
    }
    if (nAtoms < 0)
    ...
    {
        vtkErrorMacro("AcquireFileCUBE error reading file: "
                      << this->GetFileName() << " Premature EOF while reading elements.");
        file_in.close();
        return 0;
    }
    ...
    for (int i = 0; i < nAtoms; i++)
    {
        ...
        {
            vtkErrorMacro("AcquireFileCUBE error reading file: "
                          << this->GetFileName() << " Premature EOF while reading molecule.");
            file_in.close();
            return 0;
        }
    ...
    if (orbitalCubeFile)
    {
        ...
        {
            vtkErrorMacro("AcquireFileCUBE error reading file: "
                          << this->GetFileName() << " Premature EOF while reading number of orbitals.");
            file_in.close();
            return 0;
        }
        for (int k = 0; k < numberOfOrbitals; k++)
        {
            ...
            {
                vtkErrorMacro("AcquireFileCUBE error reading file: "
                              << this->GetFileName() << " Premature EOF while reading orbitals.");
                file_in.close();
                return 0;
            }
        }
    ...
    grid->GetPointData()->GetScalars()->SetName(title.c_str());
    ...
    for (int i = 0; i < n1; i++)
    {
        ...
        for (int j = 0; j < n2; j++)
        {
            for (int k = 0; k < n3; k++)
            {
                ...
                {
                    vtkErrorMacro("AcquireFileCUBE error reading file: "
                                  << this->GetFileName() << " Premature EOF while reading scalars.");
                    file_in.close();
                    return 0;
                }
...
//------------------------------------------------------------------------------
int AcquireFileCUBE::FillOutputPortInformation(int port, vtkInformation *info)
{
    if (port == 0)
    {
...
//------------------------------------------------------------------------------
void AcquireFileCUBE::PrintSelf(ostream &os, vtkIndent indent)
{
    os << indent << "CUBE parser:\n";
