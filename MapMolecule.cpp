...
// Note this class may have an accelerated Mapper subclass:
// MapMoleculeOpenGL. If you change this class please
...
{ // Params:
  ...
  // Lattice rendering is arguable here:
  RenderLattice = true;
...
/// @param //
//
void MapMolecule::UseVDWSpheresSettings()
{
  this->SetAtomicRadiusType(VDWRadius);
...
/// @param //
//
void MapMolecule::UseLiquoriceStickSettings()
{
...
/// @param //
//
void MapMolecule::UseFastSettings()
{
  this->SetAtomicRadiusType(UnitRadius);
  this->SetAtomicRadiusScaleFactor(0.5);
  this->SetAtomColorMode(DiscreteByAtom);
  //
  this->SetRenderBonds(true);
    ...
    vtkSelection *selection, vtkIdTypeArray *atomIds, vtkIdTypeArray *bondIds)
{
  ...
  // Clear the inputs: atoms
  if (atomIds != nullptr)
    atomIds->Reset();

  // Clear the inputs: bonds
  if (bondIds != nullptr)
    bondIds->Reset();
...
/// @param //
//
void MapMolecule::UpdateLatticePolyData()
{
...
/// @param //
//
void MapMolecule::setGlyphsToUpdate(bool bSet)
{
